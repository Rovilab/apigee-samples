...
kvm_name="llm-route-by-client-config"
proxy_name="llm-route-by-client"
product_name="llm-route-by-client-product"
dev_moniker="llm-route-by-client-developer"
dev_email="${dev_moniker}@acme.com"
app_name_root="llm-route-by-client"
...
if apigeecli kvms list -e "${APIGEE_ENV}" -o "$PROJECT_ID" --token "$TOKEN" | jq -e 'any(. == "'$kvm_name'")' >/dev/null; then
  ...
  #cat "$json_file"
  echo "Importing KVM to Apigee environment"
...
create_app_if_necessary "${app_name_root}-1" "$PROJECT_ID" "$product_name" "$dev_email"
apigeecli apps update --name "${app_name_root}-1" --email "${dev_email}" --attrs "modelProvider=google" --org "$PROJECT_ID" --token "$TOKEN" --disable-check >/dev/null
create_app_if_necessary "${app_name_root}-2" "$PROJECT_ID" "$product_name" "$dev_email"
apigeecli apps update --name "${app_name_root}-2" --email "${dev_email}" --attrs "modelProvider=mistral" --org "$PROJECT_ID" --token "$TOKEN" --disable-check >/dev/null
create_app_if_necessary "${app_name_root}-3" "$PROJECT_ID" "$product_name" "$dev_email"
apigeecli apps update --name "${app_name_root}-3" --email "${dev_email}" --attrs "modelProvider=huggingface" --org "$PROJECT_ID" --token "$TOKEN" --disable-check >/dev/null

APIKEY1=$(apigeecli apps get --name "${app_name_root}-1" --org "$PROJECT_ID" --token "$TOKEN" --disable-check | jq ."[0].credentials[0].consumerKey" -r)
APIKEY2=$(apigeecli apps get --name "${app_name_root}-2" --org "$PROJECT_ID" --token "$TOKEN" --disable-check | jq ."[0].credentials[0].consumerKey" -r)
APIKEY3=$(apigeecli apps get --name "${app_name_root}-3" --org "$PROJECT_ID" --token "$TOKEN" --disable-check | jq ."[0].credentials[0].consumerKey" -r)
PROXY_URL="$APIGEE_HOST/v1/samples/llm-route-by-client"
...
echo " "
echo "curl --location \"https://$APIGEE_HOST/v1/samples/llm-route-by-client/chat/completions\" \\"
...
echo "  export APIKEY1=$APIKEY1"
echo "  export APIKEY2=$APIKEY2"
echo "  export APIKEY3=$APIKEY3"
echo " "
